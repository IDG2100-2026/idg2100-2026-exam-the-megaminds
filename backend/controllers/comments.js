...
export async function getCommentById(req, res) {
    const comment = await commentService.getCommentById(req.validData.commentId);
    if (!comment) return res.status(404).json({ message: "Comment not found" });
    res.status(200).json({ success: true, data: comment });
}

export async function updateComment(req, res) {
    const updComment = await commentService.updateComment(req.validData.commentId, req.validData);
...
export async function deleteComment(req, res) {
    const delComment = await commentService.deleteComment(req.validData.commentId);
