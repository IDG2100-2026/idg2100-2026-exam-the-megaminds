...
### ⛔ Blocked / waiting
- **Live comments via WebSockets** — *your solo track* (see §4). WS is game-only today.
...
## 4. Phase 4 — live comments over WebSockets (solo guide)
...
3. **Extend the WS for tournaments** (coordinate with the `gameSocket.js` owner): add a `join-tournament` room concept + `broadcastToTournament(id, msg)`, and have the tournament-comment POST controller `broadcastToTournament(id, { type:"new-comment", comment })` after saving.
4. **`useTournamentSocket(id)`**: get `ws-token` → connect → auth → `join-tournament` → on `new-comment` append to the thread (later `standings-update`, `round-change`). Close on unmount.
